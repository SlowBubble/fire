...
function renderChordMeas(chordMeas, abcList, abcNoteDuration, measStartTime, measureEndTime) {
  ...
  chordMeas.forEach((chordLoc, idx) => {
    ...
    const cursor = chordLoc.onCursor ? '?' : '';
...
function toLyricsTokens(lyricsString) {
  ...
  const tokens = lyricsString.split(/[\s]+/).flatMap(phrase => {
    ...
    return splitAfter(phrase, asianRegexStr)
    // '天ab' => ['天', 'ab']
  }).flatMap(phrase => {
    // 'gh如' => ['gh', '如']
...
export class StateMgr {
  constructor(ebanner, urlId, shadowRoot, execPub) {
    ...
    if (mobile.isMobile()) {
      if (this.viewMode && url.href.indexOf('musicMobile.html') >= 0) {
        window.location.href = url.href.replace('musicMobile.html', 'music.html');
      }
      if (!this.viewMode && url.href.indexOf('music.html') >= 0) {
        window.location.href = url.href.replace('music.html', 'musicMobile.html');
      }
    }
  }
  ...
  truncateRight() {
    const cursorTime = this.getCursorTime();
    if (this.isChordMode()) {
      this.doc.chordLocs = this.doc.chordLocs.filter(chordLoc => {
        return chordLoc.start.lessThan(cursorTime);
      });
      return;
    }
    const noteGps = this.getCurrVoice().noteGps;
